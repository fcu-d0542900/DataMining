...
### 整理
```{r}
speed_angle <- filter(release_speed_rate,description %in% hit)[c(-3,-4)]
speed_angle <- na.omit(speed_angle)
```
```{r}
speed_angle_mean <- speed_angle %>%
  group_by(release_speed,release_spin_rate) %>%
  summarise(count = n(),mean = mean(launch_speed_angle))
```

### 繪製表格

```{r}
table_speed_angle_mean<-lapply(spin,function(r){
  data.frame(
    spin = r,
    '<70' = get(speed_angle_mean,"<70",r),
    '70-75' = get(speed_angle_mean,"70-75",r),
    '75-80' = get(speed_angle_mean,"75-80",r),
    '80-85' = get(speed_angle_mean,"80-85",r),
    '85-90' = get(speed_angle_mean,"85-90",r),
    '90-95' = get(speed_angle_mean,"90-95",r),
    '95-100' = get(speed_angle_mean,"95-100",r),
    '>100' = get(speed_angle_mean,">100",r)
  )
})%>%bind_rows()%>%t()%>%{
  colnames(.) <- spin
  .[-1,]
...
```
```{r, include=FALSE}
table_speed_angle_mean <- data.frame(table_speed_angle_mean)
colnames(table_speed_angle_mean) <- spin
rownames(table_speed_angle_mean) <- speed
```
```{r, echo=FALSE}
table_speed_angle_mean
```

### 繪製熱力圖

```{r, warning=FALSE}
heatmap.2(table_speed_angle_mean%>%as.matrix(),
          cellnote = table_speed_angle_mean%>%as.matrix(),
          notecol="black", 
          density.info="none",
          trace="none",
          col=colorpanel(24,low="white" ,high="steelblue"),
          Colv = NA, Rowv = NA)
```
